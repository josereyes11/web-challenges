...
function maxOfTwoNumbersTwo(num1, num2) {
...
function averageWordLength(words) {
  const totalLength = words
    ...
    .reduce((previousValue, currentCalue) => previousValue + currentCalue);
  const averageOfWords = totalLength / words.length;
...
function uniquifyArray(words) {
  const uniqueWords = words.filter((item, index) => {
...
function doesWordExist(haystack, needle) {
  const needleExists = haystack.includes(needle);
  return needleExists;
}
...
function howManyTimes(haystack, needle) {
  const timesNeedle = haystack.filter((item) => {
    return item === needle;
  });
  return timesNeedle.length;
}
...
function sum(array) {
  const remix = array.reduce((total, current) => {
...
function greatestProduct(matrix) {
  ...
  for (let col = 0; col <= matrix[0].length - 1; col++) {
    for (let row = 0; row <= matrix.length - 4; row++) {
      ...
      product *= matrix[row + 1][col];
      product *= matrix[row + 2][col];
      product *= matrix[row + 3][col];
      if (product > biggestProduct) {
        biggestProduct = product;
      }
    }
